...
import ProviderEnrollmentV2 from './pages/ProviderEnrollmentV2';
...
const Hero = React.memo(() => {
  return (
    <section id="hero" className="relative overflow-hidden pt-12 pb-6 lg:pt-16" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-start' }}>
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center pt-2 pb-12 sm:pt-12 sm:pb-16 lg:pt-12 lg:pb-16">
        <PhysicsBody id="hero-badge" className="inline-block mb-6">
          <motion.span
            initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
            className="inline-flex flex-row items-center gap-1 sm:gap-2.5 px-3 py-1.5 sm:px-5 sm:py-2 rounded-full border border-brand-deep/20 bg-brand-light/30 text-brand-deep text-xs sm:text-sm font-black uppercase tracking-[0.2em] backdrop-blur-md shadow-sm mb-4 whitespace-nowrap"
          >
        ...
        <PhysicsBody id="hero-title" className="mb-10">
          <h1 className="text-[clamp(3rem,6vw+0.5rem,5.5rem)] font-display font-black tracking-tighter leading-[0.95] text-slate-900">
            <motion.div
              initial="hidden"
              animate="visible"
              variants={{
        ...
        <PhysicsBody id="hero-desc" className="mb-12">
          <motion.p initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.3 }}
            className="text-[clamp(1rem,1.5vw+0.5rem,1.5rem)] leading-relaxed max-w-3xl mx-auto text-slate-500 px-2">
...
const Specialties = React.memo(() => {
  const row1 = [
    ...
    { name: "Gastroenterology", icon: ASSETS.ui.clipboard },
    { name: "Dermatology", icon: ASSETS.features.shield },
    { name: "Pain Management", icon: ASSETS.ui.activity },
  ];
  
  const row2 = [
    { name: "Internal Medicine", icon: ASSETS.specialties.general },
    { name: "Dentistry", icon: ASSETS.specialties.dentistry },
    { name: "Laboratory", icon: ASSETS.ui.target },
    ...
    { name: "OBGYN / Women's Health", icon: ASSETS.ui.userPlus },
      ...
      <PhysicsWorld>
        ...
        <div className="relative">
          ...
          <Routes>
            ...
            <Route path="/lp/provider-enrollment" element={<ProviderEnrollmentV2 />} />
            <Route path="*" element={<Home />} />
          </Routes>
